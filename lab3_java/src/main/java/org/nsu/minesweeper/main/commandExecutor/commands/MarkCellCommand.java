...
public class MarkCellCommand extends org.nsu.minesweeper.main.commandExecutor.commands.CLICommand {
    @Override
    ...
    public CommandExecuteStatus doWork(CommandData commandData, String[] args) {
        if (canExecute(commandData, args)) {
            commandData.getModel().markCell(Integer.parseInt(args[0]), Integer.parseInt(args[1]));
